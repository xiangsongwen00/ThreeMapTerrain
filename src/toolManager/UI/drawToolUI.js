...
export class DrawToolUI {
    ...
    endDrawing() {
        ...
        // 如果当前是面绘制模式且开启了贴地，更新为真正的贴地几何
        if (this.currentDrawMode === 'area' && this.isGroundDrawing) {
            // 更新为真正的贴地几何
            if (this.pickedPoints.length >= 3) {
                // 先清除之前的临时面
                this.drawTool.clearAreas();
                
                // 调用drawGroundArea并传入updateGroundGeometry: true参数
